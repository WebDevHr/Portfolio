<template>
  <div
    class="container-lg d-flex flex-column flex-lg-row justify-content-center align-items-center"
  >
    <div
      class="col-8 col-lg-4 d-felx p-3 mb-5 text-center text-lg-start my-5 my-lg-0"
    >
      <h1>Hi, I'm Hossein</h1>
      <h3>Front-end Developer who tends to be a Full-stack Developer</h3>
      <button type="button" class="btn btn-outline-success btn-lg mt-4">
        <font-awesome-icon icon="fa-solid fa-circle-down" />
        <span class="mx-2">Download CV</span>
      </button>
    </div>
    <div class="col-8">
      <img src="@/assets/pictures/developer.svg" alt="Developer" />
    </div>
  </div>
</template>

<script>
export default {};
</script>

<style></style>
